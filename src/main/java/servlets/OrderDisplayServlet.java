...
public class OrderDisplayServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (user == null) {
			...
			ordersWaiting = dao.findOrderByNullUserShipper(user.getId());
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		OrderDao dao = new OrderDao();
		String searchValueFrom;
		String searchValueTo;
		if(request.getParameter("searchValueFrom") == null || request.getParameter("searchValueTo") ==  null) {
			searchValueFrom = "1";
			searchValueTo = "5";
		}else {
			searchValueFrom = request.getParameter("searchValueFrom").toLowerCase();
			searchValueTo = request.getParameter("searchValueTo").toLowerCase();			
		}
		...
		ArrayList<Entity> allOrders = new ArrayList<Entity>();
		...
		if (user == null) {
			...
			OrderDao orderDao = new OrderDao();
			allOrders = orderDao.findOrderByUserCustomerId(user.getId());
			
		} else if (user.getRole() == Role.SHIPPER) {
			ArrayList<Order> ordersWaiting;
			ordersWaiting = dao.findOrderByUserShipperId(user.getId());
			ordersWaiting = dao.selectWaitingOrder(ordersWaiting);
			allOrders = new ArrayList<Entity>();
			for(Order oder:ordersWaiting) {
				allOrders.add((Entity)oder);
			}
			...
			allOrders = dao.findByRestorantId(restorant.getId());
		...
		disp = request.getRequestDispatcher("/sortedOrderDisplayGoodProgram.jsp");
		disp.forward(request, response);
	}
